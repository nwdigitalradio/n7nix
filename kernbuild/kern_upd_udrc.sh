...
# kern_cpy_local.sh
#
# Copy files to support udrc/udrx driver in a raspian linux source
# tree.
# Directory requirements:
#  - set source dir compass_kern
#  - set destination dir target_kern
#  - script has no location requirements
#
# DEBUG=1
# SHOW_DIFF=1
...
boot_dts_overlay_files="udrc-boost-output-overlay.dts udrc-overlay.dts udrx-overlay.dts Makefile"
...
function usage() {
   ...
   echo "   -c no arg, changes target udrc source files."
   echo "   -h no arg, display this message"
   echo
}


# ===== function modify_udrc_src
function modify_udrc_src() {

# Add udrc dts targets after uart1.dtbo
fname="arch/arm/boot/dts/overlays/Makefile"
grep -i  "udrc" "$target_kern/$kpath/$fname"
if [ $? -ne 0 ] ; then
   # instert udrc overlay targets after uart1.dtbo
   sed -i -e '/^[[:space:]]uart1.dtbo \\/a \
	udrc-boost-output.dtbo \\\
	udrc.dtbo \\\
	udrx.dtbo \\' "$target_kern/$kpath/$fname"
else
   echo "Found udrc in $fname"
fi
...
echo "Update a kernel source tree for UDRC/UDRX"
echo "Reference kernel: $compass_kern"
echo "Target kernel $target_kern"
